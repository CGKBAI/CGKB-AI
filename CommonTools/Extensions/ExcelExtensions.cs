...
public static class ExcelExtensions
{
    ...
    private static CellValues GetCellType(object item)
    {
        switch (item)
        {
            ...
            case float f:
                return float.IsNaN(f) ? CellValues.Error : CellValues.Number;

            case double d:
                return double.IsNaN(d) ? CellValues.Error : CellValues.Number;
            ...
            default:
                throw new NotSupportedException($"The type {item.GetType()} of row item is not supported");
        }
    }

    public static Row InsertNewRowInSheetData(this SheetData sheetData, uint rowIndex, IEnumerable<object> dataRow)
    {
    ...
    public static WorksheetPart InsertWorksheet(this WorkbookPart workbookPart, string sheetName, IEnumerable<IEnumerable<object>> rows, params IEnumerable<object>[] headerSets)
    {
    ...
    public static Cell InsertCellInWorksheet(this WorksheetPart worksheetPart, string columnName, uint rowIndex, string value, CellValues type)
    {
    ...
    public static WorksheetPart FreezePanelInWorksheet(this WorksheetPart worksheetPart, uint columns, uint rows)
    {
    ...
    public static WorksheetPart AddColorScalesInWorksheet(this WorksheetPart worksheetPart, uint firstColumn, uint lastColumn, uint firstRow, uint lastRow, params (double value, string color)[] scales)
    {
        ColorScale colorScale = new();

        foreach ((double value, string color) in scales)
        {
