...
public static class EnumByMatchParser<T>
    where T : Enum
{
    private static IEnumerable<TAttribute> GetAttributes<TAttribute>(T enumValue)
    {
        return EnumAnnotationHelper<T>.GetAttributes<TAttribute>(enumValue);
    }

    private static readonly IDictionary<string, T> _ciSynonymDict = EnumAnnotationHelper<T>
        .Enums
    ...
    private static readonly IDictionary<string, T> _csSynonymDict = EnumAnnotationHelper<T>
        .Enums
        .SelectMany(o => GetAttributes<MatchAttribute>(o)
            .Where(p => p.CaseSensitive && !p.UseRegex)
            .SelectMany(p => p.Patterns)
        ...
        .ToDictionary(o => o.synonym, o => o.value);
    private static readonly IList<(Regex regex, T value)> _regexSynonymList = EnumAnnotationHelper<T>
        .Enums
        .SelectMany(o => GetAttributes<MatchAttribute>(o)
            .Where(p => p.UseRegex)
    ...
    public static bool TryParse(string s, out T enumValue)
    {
        ...
        (Regex regex, T value)[] m = _regexSynonymList.Where(o => o.regex.IsMatch(s)).ToArray();
